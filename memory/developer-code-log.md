...
## 2026-03-30 凌晨3:37 | 代码练习与游戏开发（周一）
...
| 技术 | 掌握度 | 关键题目 |
|------|--------|---------|
| SRS Wall Kick | 熟练 | Tetris Raylib |
| 扫描线+主动删除堆 | 熟练 | LC218 天际线 |
| BIT+离散化 | 熟练 | LC315 统计右侧更小的数 |

### 游戏队列状态
- 队列清空 ✅
- 下周 P1: Sokoban Raylib, Flappy Bird Raylib
- 本周已超额完成
...
## 2026-03-30 早7:40 | 代码练习与游戏开发（周一）

### LeetCode 晨练

| 题目 | 分类 | 难度 | 结果 |
|------|------|------|------|
| LC529 扫雷游戏 | BFS扩散 | Medium | ✅ |
| LC417 太平洋大西洋水流 | BFS反向遍历 | Medium | ✅ |

**LC529 Minesweeper — 扫雷游戏** ✅

**题目**: 点击格子揭示：数字=相邻雷数，B=空白时BFS扩散，M直接爆炸。  
**核心算法**: BFS 简单扩散

**核心思路**:
- 点击 M → 直接揭示 X，返回
- 否则 BFS：从点击格出发，统计8方向雷数
- 有雷→显示数字；无雷→显示B，继续扩散邻居
- 用 vis 数组防止重复入队

**代码实现**:
```cpp
void bfs(board, r, c) {
    queue<pair<int,int>> q; vis[r][c]=1; q.emplace(r,c);
    while (!q.empty()) {
        auto [x,y] = q.front(); q.pop();
        int mines = countAdjMines(x,y);
        if (mines > 0) board[x][y] = '0'+mines;
        else {
            board[x][y] = 'B';
            for 8 dirs:
                if valid && !vis && board[nx][ny]=='E')
                    vis[nx][ny]=1, q.emplace(nx,ny);
        }
    }
}
```
...
**LC417 Pacific Atlantic Water Flow — 太平洋大西洋水流** ✅

**题目**: 哪些格子能同时流到太平洋（顶部+左侧）和大西洋（底部+右侧）？  
**核心算法**: BFS 从边缘反向遍历

**核心洞察**: 为什么从边缘反向？
- 正向思考（从高往低流）：每个格子的水流方向不唯一，依赖复杂
- 反向思考（从边缘向内）：只要能从边缘流过来，就一定「能流到」那个边缘
- 太平洋：处理第一行+第一列（入队，标记为可达）
- 大西洋：处理最后一行+最后一列（入队，标记为可达）
- 用两个 vis 矩阵分别追踪，合并两者都可达的格子

**代码实现**:
```cpp
// 太平洋：从左上边缘反向BFS
for (j=0;j<n;j++) { pac[0][j]=1; q.emplace(0,j); }
for (i=1;i<m;i++) { pac[i][0]=1; q.emplace(i,0); }
bfs(h, pac, q);

// 大西洋：从右下边缘反向BFS
for (j=0;j<n;j++) { atl[m-1][j]=1; q.emplace(m-1,j); }
for (i=0;i<m-1;i++) { atl[i][n-1]=1; q.emplace(i,n-1); }
bfs(h, atl, q);
...
**验证**: ✅ 输出 7 个格子：(0,4)(1,3)(1,4)(2,2)(3,0)(3,1)(4,0)

---

### 游戏开发：Sokoban Raylib ✅

**项目**: `projects/sokoban-raylib/`（已完成，编译零警告）
**Commit**: `a59c7a7` 推送完成

**功能**:
- 4 个预设关卡（Easy/Medium/Hard/Expert）
- 方向键/WASD 控制移动
- 推箱子合法性检测（不能推到墙/另一个箱子上）
- BFS 自动求解（按 B 键，最多扩展 20 万节点）
- R 重置 / N 下一关 / P 上一关
- 完成动画 + 提示
- Raylib 彩色格子渲染（墙/地板/箱子/目标/玩家）
...
| Commit | 内容 | 状态 |
|--------|------|------|
| `e808374` | .gitignore 更新（排除 Raylib 二进制）| ✅ 已推送 |
| `a59c7a7` | LC200+LC994 BFS + Sokoban Raylib 源文件 | ✅ 已推送 |
...
|------|------|------|
| 03-29 | 贪吃蛇 | Raylib 图形版 ✅ |
| 03-29 | 2048 | Raylib 图形版（Win/KeepPlaying/GameOver）✅ |
| 03-30 | 推箱子 | Raylib 图形版（BFS自动求解）✅ |
...
**BFS 反向遍历技巧**:
- 适用场景：多源可达性判定（太平洋大西洋、扫雷扩散）
- 核心思想：从已知可达边界反向扩展，比正向搜索更简单
- 太平洋：top row + left column
- 大西洋：bottom row + right column
...
|--------|------|---------|
| P1 | Flappy Bird Raylib | 重力物理 + AABB碰撞 |
| P1 | Minesweeper Raylib | BFS flood fill + 数字计算 |
| P1 | 每日 LeetCode | 图论/DP 进阶 |
| P2 | Sokoban 扩展 | 更多关卡设计（10+ 关卡）|
| P3 | WebAssembly 探索 | Emscripten 编译 C++ 游戏 |
