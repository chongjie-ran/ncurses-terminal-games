...
- Name: test.spec.js >> memory-match-wasm WASM game tests >> should load without errors
...
Expected pattern: /memory-match/i
Received string:  "Memory Match WASM"
Timeout: 5000ms

Call log:
  - Expect "toHaveTitle" with timeout 5000ms
    9 × unexpected value "Memory Match WASM"
...
```ts
  ...
  3  | 
  4  | test.describe('memory-match-wasm WASM game tests', () => {
  5  |     let server;
  6  |     
...
> 48 |         await expect(page).toHaveTitle(/memory-match/i);
     |                            ^ Error: expect(page).toHaveTitle(expected) failed
